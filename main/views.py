...
def main(requests):
    ...
    statistics = {}
    statistics["infected"] = Statistics.objects.get(
        name="infected").value + Statistics.objects.get(name="cured").value
...
def patient_admin(requests):
    ...
    raw_paths = [{"raw_paths": patient.paths, "code": patient.code}
...
def path_add(requests):
    ...
    if Patient.objects.filter(code=patient_code):
...
def get_mask_stores():
    ...
    for store in mask_stores['stores']:
        ...
        stock_at = date_parse(
            store["stock_at"]) if "stock_at" in store and store["stock_at"] else None
    ...
    log = "{}: Total searched store {}, Newly registered stores: {}".format(
        update_time, mask_stores['count'], newly_registered)
    print(log)
    # return mask_stores
...
def get_status():
    ...
    infected_num = soup.select(
        "#content > div:nth-child(2) > div > ul > li.tab-1 > div.txt2 > span:nth-child(7) > strong")

    infected_num = int(float(infected_num[0].text))

    cured_num = soup.select(
        "#content > div:nth-child(2) > div > ul > li.tab-1 > div.txt2 > span:nth-child(6) > strong")

    cured_num = int(float(cured_num[0].text))
    # 목록이 추가되면 수정하기
    names = ["infected", "cured"]

    ################### values = [‘확진자 수’, ‘완치자 수’] 가 되도록 크롤링해 주세요. ######################
    values = [infected_num, cured_num]
    # print(values)
    ...
    print(statistics)
    log = "{}: Current infected patients {}, Current cured patients: {}".format(
        updated_at, statistics['infected'],  statistics['cured'])
    print(log)


def get_patients():
    ############################# 각자의 지역에 따라 크롤링을 하셔야 합니다 ##################################
    url = "https://www.daejeon.go.kr/corona19/index.do?tab=2&subTab=2"
    headers = {
        'user-agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.132 Safari/537.36'
    }
    req = requests.get(url, headers=headers)
    ...
    table = soup.find('table', class_="corona")
    ...
    for num, row in enumerate(patients_rows):
        ...
        patients[patient_num]["ID"] = informations[0].replace("\n", "")

        patient_details = informations[3].split("(")
        patients[patient_num]["Gender"] = patient_details[0].replace("\n", "")
        patients[patient_num]["Age"] = patient_details[1][:-1]

        patients[patient_num]["Region"] = informations[4].replace("\n", "")
        patients[patient_num]["Confirmed Date"] = informations[2].replace(
            "\n", "")
        patients[patient_num]["Current Status"] = informations[6].replace(
            "\n", "")
        patients[patient_num]["Paths"] = informations[5].replace("\n", "")
    ...
    for number, patient_info in patients.items():
        ...
        if old_path != new_path:
            updated_patient.append(str(number))
    ...
    if updated_patient:
        send_mail("환자 업데이트: "+", ".join(updated_patient), ", ".join(updated_patient) +
                  "번 환자의 동선이 업데이트 되었습니다.", 'coronaulsan@gmail.com', ['coronaulsan@gmail.com'], fail_silently=False)
