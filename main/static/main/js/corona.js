...
// 대전 좌표를 중심으로 map 생성 (각 지역의 중심 좌표를 입력하세요)
var defaultx = 36.350833;
var defaulty = 127.385;
...
function searchPlaces() {
  ...
  if (currCategory == "hospital") {
    ps.keywordSearch("대전 코로나 진료소", placesSearchCB);
...
function displayPlaceInfo(place) {
  content =
    ...
    "        </div>" +
    '        <div class="w-100 p-2">' +
    '            <span class="info_content time_at">전화 번호: <strong>' +
    place.phone +
    "</strong></span>" +
    "        </div>" +
    "    </div>" +
...
var patientOverlay = new kakao.maps.CustomOverlay({
    zIndex: 1
  }),
  ...
  for (var i = 0; i < length; i++) {
    ...
    if (nodeday + nodemon * 30 > dt + mon * 30 - 30) {
      ...
      var marker = addMarker(
        new kakao.maps.LatLng(paths[i]["y"], paths[i]["x"])
      );
...
function displayPatientInfo(y, x, num, visited_date, place_name) {
  content =
    ...
    '    <div class="placeinfo pb-0">' +
    '        <div class="ptitle d-flex justify-content-between align-items-center flex-wrap">' +
    '            <h1 class="m-0 text-center"><strong>' +
    num +
    "</strong>번 확진자 동선</h1>" +
    "        </div>" +
    '        <div class="w-100 p-2">' +
    '            <span class="info_content time_at">다녀간 날짜: <strong>' +
    visited_date +
    "</strong></span>" +
    '            <span class="info_content time_at mt-1">시설 이름: <strong>' +
    place_name +
    "</strong></span>" +
    "        </div>" +
    "    </div>" +
...
var maskOverlay = new kakao.maps.CustomOverlay({
    zIndex: 1
  }),
...
function displayMask(places) {
  ...
  for (var i = 0; i < length; i++) {
    ...
    if (place_type == "01") {
      place_type = "pharmacy";
    } else if (place_type == "02") {
      place_type = "post";
    } else if (place_type == "03") {
      place_type = "mart";
    } else {
      place_type = null;
    }

    if (places[i].remain_stat == "plenty") {
      remain_stat = "100개 이상";
      color = "green";
    } else if (places[i].remain_stat == "some") {
      remain_stat = "30개 이상 100개미만";
      color = "yellow";
    } else if (places[i].remain_stat == "few") {
      remain_stat = "2개 이상 30개 미만";
      color = "red";
    } else if (places[i].remain_stat == "empty") {
      remain_stat = "1개 이하";
      color = "gray";
    } else if (places[i].remain_stat == "break") {
      remain_stat = "판매 중지";
      color = "gray";
    } else {
      remain_stat = "알 수 없음";
    ...
    var marker = addMarker(
      new kakao.maps.LatLng(places[i].latitude, places[i].longitude),
      place_type,
      color
    );
    (function(
...
function displayMaskInfo(
  ...
  if (updated_at == "None") {
    ...
    updated_at =
      updated_at.slice(0, day_index + 1) +
      noon +
      updated_at.slice(day_index + 1);
  }
  if (stock_at == "None") {
    stock_at = "알 수 없음";
  } else {
    ...
    stock_at =
      stock_at.slice(0, day_index + 1) + noon + stock_at.slice(day_index + 1);
  }
  ...
  content =
    "" +
    '    <div class="placeinfo">' +
    '        <div class="ptitle d-flex justify-content-between align-items-center flex-wrap" style="background:' +
    color +
    '">' +
    '            <h1 class="m-0">' +
    name +
    ...
    "        </div>" +
    '        <div class="w-100 p-2">' +
    '            <span class="info_content time_at">갱신 시간: <strong>' +
    updated_at +
    "</strong></span>" +
    '            <span class="info_content time_at mt-1">입고 시간: <strong>' +
    stock_at +
    "</strong></span>" +
    "        </div>" +
    '        <div class="w-100 d-flex justify-content-center px-2">' +
    '            <a class="btn w-100 p-0" href="' +
    destination_url +
    '" role="button" style="letter-spacing: 2px; background-color: #DBDBDB; opacity:0.9; color: #222; height: 30px; line-height: 30px; font-weight: 700">길찾기</a>' +
    "        </div>" +
    "    </div>" +
    '    <div class="after"></div>';
